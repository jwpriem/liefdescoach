...
<template>
	<nav role="tablist" aria-label="Account navigatie" class="fixed bottom-0 left-0 right-0 z-50 bg-gray-950/80 backdrop-blur-md border-t border-gray-800/80 flex"
		style="padding-bottom: max(calc(env(safe-area-inset-bottom) - 6px), 6px)">
		<template v-for="(tab, i) in tabs" :key="i">
			<button v-if="!(tab as any).hidden"
				role="tab"
				:id="'tab-' + tab.slot"
				:aria-selected="modelValue === i ? 'true' : 'false'"
				:aria-controls="'tabpanel-' + tab.slot"
