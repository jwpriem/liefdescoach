<template>
 <div class="bg-emerald-100 title-dark">
  <div id="weesJezelf" class="container mx-auto p-8 md:px-0 md:py-24">
   <div class="flex justify-center items-center">
    <div class="w-full md:w-1/2 md:text-center space-y-8">
     <h2 class="text-2xl md:text-4xl uppercase font-black">
              <span class="emerald-underline text-emerald-900"
              >Proefles</span
              ><span class="text-emerald-700">.</span>
     </h2>
     <p class="text-2xl text-emerald-900">
      Voor slechts <span class="text-emerald-700 font-bold">5 euro</span> kun je eenmalig een proefles boeken. Op deze manier kun je laagdrempelig ervaring op doen met Hatha Yoga of Peachy Bum.
     </p>
     <UButton color="primary" size="xl" variant="outline" to="/yoga/proefles">Proefles boeken</UButton>
    </div>
   </div>
  </div>
 </div>
</template>